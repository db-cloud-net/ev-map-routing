...
exports.NrelError = void 0;
exports.fetchDcFastChargersNearPoint = fetchDcFastChargersNearPoint;
exports.fetchElectricChargersNearPoint = fetchElectricChargersNearPoint;
exports.fetchDcFastChargersNearRoute = fetchDcFastChargersNearRoute;
class NrelError extends Error {
    constructor(message) {
        super(message);
        this.name = "NrelError";
    }
}
exports.NrelError = NrelError;
function sleepMs(ms) {
...
function parseMaxPowerKwFromStation(station) {
    const candidates = [];
    const pushIf = (v) => {
    ...
    if (Array.isArray(accessories)) {
        for (const a of accessories) {
            if (!a || typeof a !== "object")
                continue;
            pushIf(a?.max_charge_rate_kw);
            pushIf(a?.max_charge_rate);
            pushIf(a?.max_charge_rate_kW);
            pushIf(a?.power);
            pushIf(a?.max_power);
        }
...
function dedupeByLatLon(chargers, precision = 4) {
    const seen = new Set();
    const out = [];
...
async function fetchDcFastChargersNearPoint(point, radiusMiles) {
    const apiKey = process.env.NREL_API_KEY;
    if (!apiKey) {
        throw new NrelError("Missing NREL_API_KEY env var");
    }
    const baseUrl = process.env.NREL_BASE_URL ??
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        const stations = Array.isArray(json?.fuel_stations)
            ? json.fuel_stations
    ...
    throw new NrelError(`NREL charger fetch failed after ${maxAttempts} attempts${lastStatus ? ` (last status ${lastStatus})` : ""}`);
}
async function fetchElectricChargersNearPoint(point, radiusMiles) {
    const apiKey = process.env.NREL_API_KEY;
    if (!apiKey) {
        throw new NrelError("Missing NREL_API_KEY env var");
    }
...
function toWktLineString(routePoints) {
    // WKT LINESTRING expects coordinates as `X Y` => `lon lat`.
    const coords = routePoints.map((p) => `${p.lon} ${p.lat}`).join(",");
    return `LINESTRING(${coords})`;
}
async function fetchDcFastChargersNearRoute(routePoints, distanceMiles) {
    const apiKey = process.env.NREL_API_KEY;
    if (!apiKey) {
        throw new NrelError("Missing NREL_API_KEY env var");
    }
    ...
    const maxGetUrlLength = Number(process.env.NREL_NEARBY_ROUTE_MAX_GET_URL_LENGTH ?? "1800");
    const maxAttempts = Number(process.env.NREL_MAX_ATTEMPTS ?? "4");
    const interRequestDelayMs = Number(process.env.NREL_INTER_REQUEST_DELAY_MS ?? "100");
    let lastStatus = null;
    let forcedDelayMs = null;
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        const resp = fullGetUrl.length <= maxGetUrlLength
